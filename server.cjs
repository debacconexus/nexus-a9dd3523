...
app.get('/api/slides', (req, res) => {
  try {
    const fields = FIELDS.map((name, i) => ({
      field_number: i + 1,
      field_name: name,
      description: (NEXUS_META?.field_explanations?.[i]) || 'Field for ' + name,
      example_value: 'Sample value',
      nodo_tier: 'GDI',
      why_tracked: 'Data governance and audit compliance'
    }));
    
    const { generateSlideDeckHTML } = require('../schema-slides-generator.js');
    const slidesHTML = generateSlideDeckHTML(fields, NEXUS_NAME, new Date().toLocaleString());
